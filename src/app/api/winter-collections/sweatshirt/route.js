...
export const GET = async (req) => {
    ...
    const shacketCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            category: { $regex: /^sweatshirt$/i }, // Case-insensitive regex query for category
        ...
        const totalProducts = await shacketCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await shacketCollection.find(query)
