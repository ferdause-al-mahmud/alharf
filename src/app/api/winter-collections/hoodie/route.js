...
export const GET = async (req) => {
    ...
    const hoodieCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            category: { $regex: /^hoodie$/i }, // Case-insensitive regex query for category
        ...
        const totalProducts = await hoodieCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await hoodieCollection.find(query)
