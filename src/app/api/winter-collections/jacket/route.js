...
export const GET = async (req) => {
    ...
    const jacketCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            category: { $regex: /^jacket$/i }, // Case-insensitive regex query for category
        ...
        const totalProducts = await jacketCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await jacketCollection.find(query)
