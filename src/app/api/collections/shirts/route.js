...
export const GET = async (req) => {
    ...
    const shirtCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            category: { $regex: /^shirts$/i }, // Case-insensitive regex query for category
        ...
        const totalProducts = await shirtCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await shirtCollection.find(query)
