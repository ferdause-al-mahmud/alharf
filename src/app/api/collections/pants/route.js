...
export const GET = async (req) => {
    ...
    const pantsCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            subcategory: { $regex: /^casual-pants$/i }, // Case-insensitive regex query for category
        ...
        const totalProducts = await pantsCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await pantsCollection.find(query)
