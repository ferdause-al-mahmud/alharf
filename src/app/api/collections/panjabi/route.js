...
export const GET = async (req) => {
    ...
    const panjabiCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            category: { $regex: /^panjabi$/i }, // Case-insensitive regex query for category
        ...
        const totalProducts = await panjabiCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await panjabiCollection.find(query)
