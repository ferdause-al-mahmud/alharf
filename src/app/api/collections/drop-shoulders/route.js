...
export const GET = async (req) => {
    ...
    const dropShoulderCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            category: { $regex: /^drop-shoulders$/i }, // Direct string match instead of regex for now
            price: { $gte: minPrice, $lte: maxPrice } // Make sure price is a number in the DB
        };
        ...
        if (sortBy === 'lowToHigh') {
            ...
            // Default case (if sortBy is not recognized), sort by newest
            sortOption = { createdAt: -1 }; // Sort by newest first
        }

        // Count total products that match the query
        const totalProducts = await dropShoulderCollection.countDocuments(query);
        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await dropShoulderCollection.find(query)
