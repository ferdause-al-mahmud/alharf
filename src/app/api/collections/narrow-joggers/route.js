...
export const GET = async (req) => {
    ...
    const joggerCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            subcategory: { $regex: /^narrow-joggers$/i }, // Case-insensitive regex query for category
            price: { $gte: minPrice, $lte: maxPrice } // Price filter
        };
        ...
        const totalProducts = await joggerCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await joggerCollection.find(query)
