...
export const GET = async (req) => {
    ...
    const salesCollection = db.collection('products');
    ...
    try {
        ...
        const query = {
            type: { $in: [/^sale$/i] }, // case-insensitive "featured"
        ...
        const totalProducts = await salesCollection.countDocuments(query);

        // Calculate the number of products to skip for pagination
        const skip = (page - 1) * limit;

        // Fetch the paginated and sorted products
        const featuredProducts = await salesCollection.find(query)
